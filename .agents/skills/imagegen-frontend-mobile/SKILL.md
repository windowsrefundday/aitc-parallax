...
Before generating any images, **infer what the user actually wants**. Most LLM design output is bad because the model jumps to a default aesthetic instead of reading the room.

### 0.A Read these signals first
1. **App kind** - ecommerce, fintech, wellness, productivity, social.
2. **Vibe words** - "minimalist", "calm", "premium consumer", "playful", "trust-first".
3. **Reference signals** - URLs, screenshots, named competitors.
4. **Audience** - gen-z consumer vs. B2B enterprise vs. accessibility-first.

### 0.B Output a one-line "Design Read" before generating
Before generating, state in one line: **"Reading this as: <app kind> for <audience>, with a <vibe> language, leaning toward <aesthetic family>."**
Example: *"Reading this as: premium consumer ecommerce app for fashion buyers, leaning toward editorial typography and minimal Chrome."*

### 0.C Anti-Default Discipline
Do not default to: AI-purple gradients, three equal feature cards, generic glassmorphism on everything, Inter + slate-900. These are the LLM defaults. Reach past them deliberately based on the design read.
...
* **SHAPE CONSISTENCY LOCK (mandatory):** Pick ONE corner-radius scale for the app and stick to it. Options: all-sharp, all-soft (12-16px), all-pill. Round buttons in a square layout is broken design.
* **EYEBROW RESTRAINT (mandatory):** An "eyebrow" is the small uppercase wide-tracking label sitting above a section headline. Maximum 1 eyebrow per 3 screens. If screen A has an eyebrow, the next 2 screens cannot have one.
* **BENTO BACKGROUND DIVERSITY (mandatory):** Bento and feature-grid sections cannot be just white-on-white cards with text inside. Use a real image, a brand-appropriate gradient (not AI-purple), a pattern, or a tinted background for at least 2-3 cells.
...
* **THE LILA RULE (NO AI PURPLE):** The "AI Purple / Blue glow" aesthetic is discouraged as a default. No automatic purple button glows, no random neon gradients. Use neutral bases with high-contrast singular accents.
* **COLOR CONSISTENCY LOCK (mandatory):** Once an accent color is chosen for a flow, it is used on the WHOLE flow. Pick one accent, lock it, audit every component before shipping.
* **PREMIUM-CONSUMER PALETTE BAN (mandatory):**
  - For premium-consumer briefs (wellness, artisan, luxury, DTC), the default AI palette of **warm beige/cream + brass/clay/oxblood + espresso dark text** is BANNED.
  - Every premium-consumer site the LLM defaults to uses this palette (`#f5f1ea` + `#b08947` + `#1a1714`). The brand becomes invisible.
...
* **CTA BUTTON WRAP BAN (mandatory):** Button text MUST fit on one line. If a label wraps to 2 lines, the button is broken. Keep labels short (1-3 words max).
* **NO DUPLICATE CTA INTENT (mandatory):** Two CTAs with the same intent on one screen is a Pre-Flight Fail. Pick ONE label per intent.
* **COPY SELF-AUDIT (mandatory):** AI-generated cute copy is worse than boring copy. Avoid fake-precise numbers like `92%`, `4.1x`, `13.4 lb` unless they come from real data. Don't fake engineering precision the brand doesn't claim.
- short
- clean
- product-appropriate
- readable
...
* **SERIF DISCIPLINE (VERY DISCOURAGED AS DEFAULT):**
  ...
  - Serif is only acceptable when the brand brief literally names a serif font, OR the aesthetic family is genuinely editorial / luxury / heritage AND you can articulate why it fits.
  - For everything else, **default sans-serif display** (Geist, Cabinet Grotesk, Inter Tight).
  - **Specifically BANNED as defaults:** `Fraunces` and `Instrument_Serif` (the two LLM-favorite display serifs).
* **EMPHASIS RULE:** When you want to emphasize a word within a headline, use **italic or bold of the SAME font**. Do NOT inject a random serif word into a sans headline just to add visual interest. Mixed-family emphasis is amateur.
